...
func _on_actualizar_info_meteoritos(numero : int) -> void:
	...
	info_meteoritos.modificar_texto("Meteoritos\nRestantes\n{cantidad}".format({"cantidad" : numero}))
...
func _on_actualizar_energia_laser(energia_max : float , energia_actual) -> void:
...
func _on_nave_destruida(nave : NaveBase, _posicion : Vector2, _explosiones) -> void:
